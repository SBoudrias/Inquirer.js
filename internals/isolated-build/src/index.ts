...
class IsolatedBuild {
  ...
  constructor(packageName: string, verbose: boolean = false) {
  ...
  private packWorkspaces(dependencies: Set<string>): Map<string, string> {
  ...
  private createIsolatedEnvironment(
    packageName: string,
  ...
  private run(packageName: string): void {
    try {
