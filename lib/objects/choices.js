...
var Choices = (function () {
    function Choices(choices, answers) {
        this.choices = choices.map(function (val) {
            if (val.type === 'separator') {
                if (!(val instanceof separator_1.Separator)) {
                    val = new separator_1.Separator(val.line);
                }
                return val;
            }
            return new choice_1.Choice(val, answers);
        });
        this.realChoices = this.choices
            .filter(separator_1.Separator.exclude)
    ...
    Choices.prototype.push = function () {
        var objs = _.map(arguments, function (val) {
            return new choice_1.Choice(val);
        });
        this.choices.push.apply(this.choices, objs);
        this.realChoices = this.choices.filter(separator_1.Separator.exclude);
        return this.choices;
    };
    return Choices;
}());
