...
Prompt.prototype.getChoices = function() {
  ...
  this.opt.choices.forEach(function( choice, i ) {
    ...
    var isSelected = (i - separatorOffset === this.pointer);
...
Prompt.prototype.onSubmit = function() {
  var choices = _.where(this.opt.validChoices, { checked: true });
...
Prompt.prototype.onKeypress = function( s, key ) {
  ...
  var len = this.opt.validChoices.length;
  this.rl.output.unmute();

  if ( key.name === "space" ) {
    var checked = this.opt.validChoices[ this.pointer ].checked;
    this.opt.validChoices[ this.pointer ].checked = !checked;
  } else if ( key.name === "up" ) {
