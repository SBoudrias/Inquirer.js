...
Prompt.prototype.validate = function() {
  ...
  if ( formatError ) {
    throw new Error("`key` param must be a single letter and is required.");
  }
  if ( keymap.h ) {
    throw new Error("`key` param cannot be `h` - this value is reserved.");
  }
  if ( errors.length ) {
    throw new Error( "`key` param must be unique. Duplicates: " +
        _.uniq(errors).join(", ") );
  }
};
