...
ScreenManager.prototype.render = function (content, opt) {
  ...
  var prompt = lines[lines.length - 1].replace(this.rl.line, '');
  lines = _.flatten(breakedLines);
  this.height = lines.length;
  this.rl.setPrompt(prompt);
  this.rl.output.write(content);

  // Re-adjust cursor at the end of the current prompt
  var promptLineUpDiff = Math.floor(stripAnsi(prompt + this.rl.line).length / cliWidth()) - cursorPos.rows;
  util.up(this.rl, opt.cursor + promptLineUpDiff);

  // Reset cursor at the beginning of the line
  util.left(this.rl, stripAnsi(_.last(lines)).length);
