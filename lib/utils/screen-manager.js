...
ScreenManager.prototype.render = function (content, opt) {
  ...
  // TODO: escape the line as regex. We only want to match the rl.line ending
  // the current line. If we don't we'll match on prompt letters when the input
  // is small, and we'll confuse the cursor position
  var prompt = promptLine.replace(new RegExp(this.rl.line + '$', 'gm'), '');
