...
ScreenManager.prototype.render = function (content, bottomContent) {
  ...
  var promptLine = _.last(content.split(/\n/));
  ...
  var lastLine = _.last(fullContent.split('\n'))
  util.left(this.rl, stripAnsi(lastLine).length);

  // Adjust cursor on the right
  var rightPos = cursorPos.cols;
  if (cursorPos.rows === 0) {
    rightPos = Math.max(rightPos, stripAnsi(_.last(content.split('\n'))).length);
  }
   ...
   */
  this.extraLinesUnderPrompt = bottomContent ? bottomContent.split('\n').length : 0;
  this.height = fullContent.split('\n').length;
