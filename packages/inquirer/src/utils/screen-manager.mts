...
export default class ScreenManager {
  ...
  height: number = 0;
  extraLinesUnderPrompt: number = 0;

  rl: InquirerReadline;
  spinnerId?: NodeJS.Timeout;

  constructor(rl: InquirerReadline) {
    this.rl = rl;
  }

  renderWithSpinner(content: string, bottomContent: string) {
    if (this.spinnerId) {
    ...
    this.spinnerId = setInterval(
      ...
      // @ts-expect-error Change in new ora API messing up older version usage
      spinner.interval,
    );
  }

  render(content: string, bottomContent: string, spinning = false) {
